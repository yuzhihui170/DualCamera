...
public class MediaEncoderHandler implements Runnable {
    ...
    private static final int MSG_TAKEPICTURE = 6;
...
//    private VideoEncoderCore mVideoEncoder;
    ...
    public static class EncoderConfig {
        ...
        public EncoderConfig(Context context , String outputPath, int width, int height, int bitRate, EGLContext sharedEglContext) {
            mOutputPath = outputPath;
            mWidth = width;
            mHeight = height;
            mBitRate = bitRate;
        ...
        public String toString() {
            return "EncoderConfig: " + mWidth + "x" + mHeight + " @" + mBitRate + " to '" + mOutputPath + "' ctxt=" + mEglContext;
        }
    }
    ...
    public void takePicture() {
    	mHandler.removeMessages(MSG_FRAME_AVAILABLE);
    	mHandler.sendMessage(mHandler.obtainMessage(MSG_TAKEPICTURE));
    }
        ...
        public void handleMessage(Message inputMessage) {
            ...
            switch (what) {
                ...
                case MSG_TAKEPICTURE:
                	encoder.handleTakePicture(null);
                	break;
...
//        drawBox(mFrameNum++);
    ...
    private void handleTakePicture(File file) {
    	SimpleDateFormat sDateFormat = new SimpleDateFormat("yyyyMMddhhmmss");       
    	String date = sDateFormat.format(new java.util.Date());  
    	try {
			mInputWindowSurface.saveFrame(new File("/sdcard/upixels/pic_" + date + ".jpg"));
		} catch (IOException e) {
			e.printStackTrace();
		}
    }
...
//        mFullScreen = new FullFrameRect(new Texture2dProgram(Texture2dProgram.ProgramType.TEXTURE_EXT));
    ...
    private void prepareEncoder(EGLContext sharedContext, int width, int height, int bitRate, String outputPath) {
        try {
            final MediaMuxerWrapper muxer = new MediaMuxerWrapper(outputPath);
...
//                    synchronized (mSync) {
//                        if (mUVCCamera != null) {
//                            mUVCCamera.stopCapture();  //停止录像,底层停止将数据拷贝到surface
//                        }
//                    }
