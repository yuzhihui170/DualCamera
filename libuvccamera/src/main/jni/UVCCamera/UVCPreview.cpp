...
static int directCopyToSurface(uvc_frame_t *frame, ANativeWindow **window);
static int isValidRGBA(void *data);
...
uvc_frame_t *UVCPreview::get_frame(size_t data_bytes) {
	...
	if UNLIKELY(!frame) {
		LOGW("allocate new frame data_bytes(%zd)", data_bytes);
...
//#endif
	...
	if (LIKELY(preview->isRunning())) {
		uvc_frame_t *copy = preview->get_frame(frame->width * frame->height * 4); //frame->data_bytes
		if (UNLIKELY(!copy)) {
#if LOCAL_DEBUG
...
int UVCPreview::prepare_preview(uvc_stream_ctrl_t *ctrl) {
	...
	if (LIKELY(!result)) {
		...
		if (LIKELY(!result)) {
			...
			LOGW("frameSize=(%d,%d)@%s", frameWidth, frameHeight, (!requestMode ? "YUYV" : "MJPEG"));
			pthread_mutex_lock(&preview_mutex);
			if (LIKELY(mPreviewWindow)) {
...
void UVCPreview::do_preview(uvc_stream_ctrl_t *ctrl) {
	...
	if (LIKELY(!result)) {
		...
		LOGI("Streaming...");
#endif
        LOGW("Streaming... frameMode(%d)",frameMode);
		if (frameMode) {
			// MJPEG mode
			for ( ; LIKELY(isRunning()) ; ) {
				...
				if (LIKELY(frame_mjpeg)) {
					...
					result = uvc_mjpeg2rgbx(frame_mjpeg, frame);   // MJPEG => rgba
					recycle_frame(frame_mjpeg);
					if (LIKELY(!result)) {
                        if (isValidRGBA(frame->data) == 1) {
                            directCopyToSurface(frame, &mPreviewWindow);
                        } else {
							LOGW("ignore invalid rgba data!!");
						}
...
// direct transfer specific frame data to the Surface(ANativeWindow)
static int directCopyToSurface(uvc_frame_t *frame, ANativeWindow **window) {
    // ENTER();
    int result = 0;
    if (LIKELY(*window)) {
        ...
        if (LIKELY(ANativeWindow_lock(*window, &buffer, NULL) == 0)) {
            ...
            // use lower height
            const int h = frame->height < buffer.height ? frame->height : buffer.height;
//            MARK("frame w(%d) h(%d) data_bytes(%d) buffer w(%d) h(%d) format(%d) stride(%d)",
//				   frame->width, frame->height, frame->data_bytes, buffer.width, buffer.height, buffer.format, buffer.stride);
            // transfer from frame data to the Surface
            copyFrame(src, dest, w, h, src_step, dest_step); // <RGBA>copyFrame time = 2ms, dest_step = 13312
            ANativeWindow_unlockAndPost(*window);
...
//判断是否是有效的RGBA数据 1:有效 -1:无效 将一系列数据相加,如果都是0的话,那么就是无效数据
static int isValidRGBA(void *data) {
    char *tmp = (char *)data;
    int remove_black =  tmp[300*960+320] + tmp[310*960+330] + tmp[330*960+340] +
                        tmp[340*960+350] + tmp[350*960+360] + tmp[360*960+370] +
                        tmp[370*960+350] + tmp[380*960+360] + tmp[390*960+370] +
                        tmp[400*960+320] + tmp[410*960+360] + tmp[420*960+370] +
                        tmp[430*960+50] + tmp[450*960+360] + tmp[480*960+370] +
                        tmp[451*960+400] + tmp[494*960+200] + tmp[510*960+370];
    if (remove_black == 0) {
        return -1;
